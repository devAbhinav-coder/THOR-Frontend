...
function stripDangerous(html: string): string {
  return html
    ...
    .replace(
      /<(iframe|object|embed|svg|math|link|meta|base|form)[\s\S]*?>[\s\S]*?<\/\1>/gi,
      "",
    )
    .replace(
      /<(iframe|object|embed|svg|math|link|meta|base|form)[^>]*\/?>/gi,
      "",
    )
...
function openTag(tag: string, attrsRaw: string): string {
  ...
  if (
    ["p", "h1", "h2", "h3", "blockquote", "figure", "figcaption"].includes(tag)
  ) {
