...
export const HOME_H1 = "Premium Sarees, Salwar Suits & Corsets Online in India";
...
const GENERIC_SHOP_BANNER_TITLES = new Set(
  ["shop our collection", "shop collection", "all sarees"].map((s) =>
    s.toLowerCase(),
  ),
);
...
export function resolveShopBannerH1(options: {
  ...
  if (options.isFeatured === "true")
    return "Featured Sarees, Salwar Suits & Corsets Online India";
  if (options.onSale === "true")
    return "Sale Sarees, Salwar Suits & Corsets Online India";
  const banner = String(options.bannerTitle || "").trim();
  if (banner && !GENERIC_SHOP_BANNER_TITLES.has(banner.toLowerCase()))
    return banner;
...
export function resolveGiftingHeroH1(heroTitle?: string | null): string {
  ...
  if (!t || GENERIC_GIFTING_TITLES.has(t.toLowerCase()))
    return GIFTING_H1_DEFAULT;
