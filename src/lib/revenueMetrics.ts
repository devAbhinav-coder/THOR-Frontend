...
export function buildFinancialSnapshot(
  ...
  const grossRevenue =
    usePeriod && overview.grossRevenue != null ?
      overview.grossRevenue
    : (overview.totalRevenue ?? overview.grossRevenue ?? 0);

  const refunds =
    usePeriod && overview.refunds != null ? overview.refunds : (refunded ?? 0);
  ...
  const productRevenue =
    usePeriod && overview.productRevenue != null ?
      overview.productRevenue
    : (overview.productRevenue ?? 0);

  const cogs =
    usePeriod && overview.cogs != null ?
      overview.cogs
    : (overview.productCogs ?? overview.cogs ?? 0);

  const grossProfit =
    usePeriod && overview.grossProfit != null ?
  ...
  const netIncomeMarginPct =
    productRevenue > 0 ? (netIncome / productRevenue) * 100 : 0;
