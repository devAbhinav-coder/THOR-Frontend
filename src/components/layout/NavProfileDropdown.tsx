...
export default function NavProfileDropdown({
  ...
  const showAdminPanel = isAdminPanelUser(panelUser);
  const adminHomeHref = defaultAdminHomeHref(panelUser);

  const items = PROFILE_MENU_ITEMS.filter(
    (item) => !item.adminOnly || showAdminPanel,
  ).map((item) =>
    item.adminOnly ? { ...item, href: adminHomeHref } : item,
  );
      ...
      <div className={cn(navLuxuryDropdownHeader, "pb-6 pt-6")}>
        <div className="mx-auto mb-3 flex h-16 w-16 items-center justify-center overflow-hidden rounded-full border-2 border-[#c5a059] bg-navy-800">
          <ProfileAvatarImg
            name={displayName}
            avatar={user.avatar}
            resetKey={user.email ?? displayName}
            initialsClassName="font-serif text-xl font-medium text-[#c5a059]"
          />
        </div>
