...
import { Bot, ExternalLink, Mail, PhoneCall, Trash2, X } from "lucide-react";
...
import { useRaniCareLayout } from "./useRaniCareLayout";
import { useDraggableFabPosition } from "./useDraggableFabPosition";
...
export function RaniCareChatPanel(props: ChatProps) {
  ...
  const { hasMobileBottomNav } = useRaniCareLayout();
  const { position, isDragging, fabProps } = useDraggableFabPosition({
    hasMobileBottomNav,
    onOpen: () => setOpen(true),
  });
  ...
  return (
    <>
      ...
      {!open && position && (
        ...
        >
          <RaniCareFab isDragging={isDragging} {...fabProps} />
        </div>
      )}
      ...
      {open && (
        <div
          className={cn(
            "fixed z-[95] flex flex-col",
            "inset-x-0 bottom-0 max-lg:max-h-[min(88dvh,100dvh)]",
            "lg:inset-auto lg:bottom-5 lg:right-5 lg:w-[min(100vw-2rem,380px)]",
          )}
        >
          <div
            data-lenis-prevent
            ...
            className={cn(
              ...
              "max-lg:h-[min(88dvh,100dvh)] max-lg:max-h-[min(88dvh,100dvh)] max-lg:rounded-none",
              "lg:h-[min(640px,78vh)] lg:shadow-[0_24px_60px_-12px_rgba(20,25,47,0.28)]",
              isClosing ?
          ...
          >
              ...
              <div className='flex items-center justify-between gap-2'>
                <div className='flex min-w-0 items-center gap-2.5'>
                  ...
                  <div className='min-w-0'>
                    <h3 className='truncate font-serif text-[15px] font-semibold text-white'>
                      Rani Care
                    </h3>
            ...
            <div
              ref={scrollRef}
              className={cn(
                "min-h-0 flex-1 overflow-y-auto overscroll-y-contain px-3 py-3 scrollbar-hide touch-pan-y",
                raniBodyBg,
              )}
            >
                ...
                {messages.length === 0 && !typing && (
                  ...
                  >
                    ...
                    <p className='mt-1 text-[11px] text-gray-500'>
                      Orders, delivery, returns, or styling — just ask.
                    </p>
            ...
            <footer className='shrink-0 border-t border-gray-200 bg-white px-3 py-2.5 pb-[max(0.625rem,env(safe-area-inset-bottom))]'>
              <RaniCareComposer
                input={input}
                setInput={setInput}
                ...
                focusOnMount={open}
              />
              <div className='mt-2 flex items-center justify-between gap-3 text-[10px] uppercase tracking-[0.12em] text-gray-400'>
                ...
                <Link
                  ...
                  className='inline-flex shrink-0 items-center gap-0.5 font-semibold text-[#c5a059] transition-colors hover:text-[#b8924d]'
                >
