...
import { BLOG_CATEGORIES, blogExcerpt, categoryLabel } from "@/lib/blogHelpers";
import {
  gridColSpan,
  gridVariantForIndex,
  listingImageAspect,
...
function GridCard({ blog, index }: { blog: Blog; index: number }) {
  ...
  if (variant === "horizontal") {
    return (
      <article
        className={`${gridColSpan(variant)} flex flex-col md:flex-row gap-8 journal-hover-lift group`}
      >
        <Link
          href={`/blog/${blog.slug}`}
...
export default function BlogListingClient({
  ...
  const { data, isLoading, isFetchingNextPage, fetchNextPage, hasNextPage } =
    useInfiniteQuery<{
      ...
      initialData:
        !hasActiveFilters && initialBlogs.length > 0 ?
          {
            pages: [
              {
                blogs: initialBlogs,
  ...
  const statusLabel = (() => {
    ...
    if (isSearching || (isLoading && blogs.length > 0))
        ...
        >
          <Search
            className={cn(
              ...
              : "text-account-on-surface-variant/50",
            )}
            aria-hidden
          />
          <input
          ...
          <div className='text-center py-24 border border-dashed border-account-outline-variant/40'>
            ...
            {!hasActiveFilters && (
              <p className='text-account-on-surface-variant/80'>
                ...
                <Link
                  href='/shop'
                  className='text-account-secondary hover:underline'
                >
                  collection
                </Link>{" "}
                in the meantime.
