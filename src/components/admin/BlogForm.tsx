...
export default function BlogForm({ initialData, prefill }: BlogFormProps) {
  ...
  const handleInsertInStory = (index: number) => {
    ...
    if (ok) toast.success(`Image ${index + 1} inserted into the story`);
      ...
      toast.error(
        "Could not insert - image may already be in the story or is the featured cover",
      );
  };
  ...
  const handleSetGallery = (index: number) => {
    setGalleryRows((prev) =>
      prev.map((row, i) =>
        i === index ? { ...row, placement: "gallery" as const } : row,
      ),
    );
    toast.success(`Image ${index + 1} added to end gallery`);
  };
  const [pendingCropFiles, setPendingCropFiles] = useState<File[]>([]);
  const [pendingCropLayout, setPendingCropLayout] =
    useState<BlogImageLayout>("hero");
  const [pendingCropPreviewUrl, setPendingCropPreviewUrl] = useState<
    string | null
  >(null);
  ...
  const handleQuickUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    toast.success(
      `${filesArray.length} image(s) added - first image is cover (Hero)`,
    );
    e.target.value = "";
  };
  ...
  const handleCropComplete = (croppedBlob: Blob | File) => {
    ...
    if (croppedBlob instanceof File) {
      ...
      newFile = new File([croppedBlob], currentFile.name, {
        type: "image/jpeg",
      });
    }
  ...
  const handleCropCancel = () => {
    ...
    setPendingCropFiles((prev) => {
      ...
      if (remaining.length > 0) {
        toast("Skipped image - crop the next one or use Quick upload.", {
          icon: "ℹ️",
        });
      }
      return remaining;
  ...
  const handleRemoveExistingImage = async (publicId: string) => {
    ...
    if (
      !confirm(
        "Are you sure you want to permanently delete this image from the server?",
      )
    ) {
      return;
    }
    try {
      ...
      setGalleryRows((prev) =>
        prev.filter(
          (row) => !(row.kind === "existing" && row.publicId === publicId),
        ),
      );
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      if (aiPromptSnapshot)
        formData.append("aiPromptSnapshot", aiPromptSnapshot);
      ...
      if (isEditing) {
        ...
        if (newRows.length > 0) {
          ...
          formData.append(
            "newCaptions",
            JSON.stringify(newRows.map((r) => r.caption)),
          );
          formData.append(
            "newLayouts",
            JSON.stringify(newRows.map((r) => r.layout)),
          );
          formData.append(
            "newPlacements",
            JSON.stringify(newRows.map((r) => r.placement)),
          );
    ...
    onSuccess: (res) => {
      const savedImages = (res as { data?: { blog?: { images?: unknown[] } } })
        ?.data?.blog?.images;
      const newCount = galleryRows.filter((r) => r.kind === "new").length;
      if (newCount > 0 && (!savedImages || savedImages.length === 0)) {
        toast.error(
          "Blog saved but images may be missing - please edit and re-upload.",
        );
      } else {
        toast.success(
          `Blog ${isEditing ? "updated" : "created"} successfully!`,
        );
      }
    ...
    onError: (err: any) => {
      toast.error(
        err.message || `Failed to ${isEditing ? "update" : "create"} blog`,
      );
    },
  });
  ...
  const applyAiDraft = (draft: BlogCopyDraft) => {
    ...
    if (draft.content) {
      ...
      setContent(
        normalizeBlogStorageContent(
          injectImagesIntoContent(draft.content, slots),
        ),
      );
    }
    ...
    if (draft.linkProductIds?.length)
      setRelatedProductIds(draft.linkProductIds);
    setAiGenerated(true);
  };
        ...
        <div className='flex flex-col sm:flex-row items-start sm:items-center justify-between border-b border-gray-100 pb-8 gap-6'>
          <div>
            ...
            <p className='text-sm text-gray-500 mt-2 font-medium'>
              Craft a beautiful story complete with vibrant imagery.
            </p>
          </div>
        ...
        <div className='grid md:grid-cols-2 gap-8'>
          ...
          <div className='space-y-2 md:col-span-2'>
            <label className='block text-sm font-bold text-gray-700 uppercase tracking-wide flex items-center justify-between'>
              <span>
                Slug <span className='text-brand-500'>*</span>
              </span>
          ...
          <div className='md:col-span-2'>
            ...
            >
              ...
              <span className='flex items-center gap-2'>
                ...
                <ChevronDown
                  className={`w-4 h-4 text-emerald-700 transition-transform ${seoOpen ? "rotate-180" : ""}`}
                />
              </span>
            </button>
            {seoOpen && (
                ...
                {pendingCropFiles.length > 0 && (
                  <div className='p-3 rounded-xl bg-amber-50 border border-amber-200 text-xs text-amber-800'>
                    <p className='font-medium'>
                      {pendingCropFiles.length} waiting to crop
                    </p>
                    <button
                      type='button'
                ...
                <details className='rounded-xl border border-gray-200 bg-white'>
                  ...
                  <div className='px-3 pb-3'>
                    <BlogImageGalleryEditor
                      ...
                      onRemoveExisting={
                        isEditing ? handleRemoveExistingImage : undefined
                      }
                      onInsertInStory={handleInsertInStory}
                      onSetCover={handleSetCover}
                      maxImages={10}
