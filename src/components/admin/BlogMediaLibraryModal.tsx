...
import AdminFullScreenOverlay, {
  overlayScrollClass,
} from "./AdminFullScreenOverlay";
...
export default function BlogMediaLibraryModal({
  ...
  const selectedItem =
    selected != null ? enriched.find((i) => i.index === selected) : null;
  ...
  const filters: { id: Filter; label: string; count: number }[] = [
    { id: "all", label: "All media", count: enriched.length },
    {
      id: "featured",
      label: "Featured",
      count: enriched.filter((i) => i.placement === "cover").length,
    },
    {
      id: "available",
      label: "Available",
      count: enriched.filter((i) => i.placement !== "cover" && !i.inStory)
        .length,
    },
    {
      id: "in-story",
      label: "In story",
      count: enriched.filter((i) => i.inStory).length,
    },
    {
      id: "gallery",
      label: "End gallery",
      count: enriched.filter((i) => i.placement === "gallery").length,
    },
  ];
  ...
  return (
    <AdminFullScreenOverlay
      open={open}
      className='bg-[#0f1117]/98 backdrop-blur-md'
    >
      {/* Header */}
        ...
        <div className='flex items-center gap-3'>
          ...
          <div>
            ...
            <p className='text-xs text-white/50'>
              Select images to insert, set as featured, or add to the end
              gallery
            </p>
          </div>
        </div>
          ...
          {enriched.length === 0 ?
            <div className='flex flex-col items-center justify-center h-full min-h-[320px] text-center'>
              ...
              <p className='text-sm text-white/50 max-w-sm mb-6'>
                Upload photos to your media library. The first image becomes the
                featured cover automatically.
              </p>
              <button
                type='button'
                onClick={onUploadRequest}
          ...
          <aside className='w-72 shrink-0 border-l border-white/10 bg-[#151820] p-5 space-y-4 hidden lg:flex flex-col'>
            ...
            {selectedItem.placement === "cover" && (
              <p className='text-xs text-white/40 leading-relaxed'>
                Featured images appear in the article hero header. Use a
                different image to insert into the story body.
              </p>
            )}
          </aside>
        )}
