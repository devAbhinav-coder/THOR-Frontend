...
export default function InventorySnapshotPanel({
  ...
  const metrics = [
    ...
    },
    {
      ...
      color:
        valuation.potentialMargin >= 0 ? "text-emerald-700" : "text-red-600",
    },
    {
      label: "Total units",
    ...
    <section className='rounded-[1.5rem] border border-gray-200/80 bg-white/80 backdrop-blur-sm shadow-sm hover:shadow-md transition-shadow duration-300 overflow-hidden group'>
      ...
      >
        {metrics.map((c) => (
          ...
          >
            <p className='text-[10px] font-semibold text-gray-400 uppercase tracking-wide group-hover:text-brand-500 transition-colors'>
              {c.label}
            </p>
            <p className={cn("text-lg font-bold mt-1 tabular-nums", c.color)}>
              {c.value}
            </p>
