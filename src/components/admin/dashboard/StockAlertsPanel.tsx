...
export default function StockAlertsPanel({
  ...
  const totalAlerts =
    (stockHealth?.outOfStock ?? outOfStock.length) +
    (stockHealth?.lowStock ?? lowStockOnly.length);
    ...
    >
      <div className='px-5 py-4 border-b border-gray-100 flex items-start justify-between gap-3'>
        <div>
          ...
          <p className='text-xs text-gray-500 mt-0.5'>
            Out of stock and low stock (&lt;{LOW_STOCK_ALERT_EXCLUSIVE_MAX}{" "}
            units) are tracked separately
          </p>
        </div>
        {showManageLink && (
          <Link
      ...
      {allClear ?
        ...
        >
          ...
          <p className='text-sm font-medium text-gray-800'>
            All active SKUs are well stocked
          </p>
          <p className='text-xs text-gray-500 mt-1'>
            No out-of-stock or low-stock alerts right now
          </p>
        </div>
      : <div
          className={cn(
        ...
        >
          <div className='p-4 sm:p-5 min-h-0'>
            ...
            {(stockHealth?.outOfStock ?? 0) > outOfStock.length && (
              <p className='text-[10px] text-gray-400 mt-2'>
                Showing top {outOfStock.length} of {stockHealth?.outOfStock} -
                see Inventory for full list
              </p>
            )}
          </div>
          ...
          <div className='p-4 sm:p-5 min-h-0'>
            ...
            {lowStockOnly.length === 0 ?
              <p className='text-sm text-gray-500 py-4'>
                No low-stock items (1–{LOW_STOCK_ALERT_EXCLUSIVE_MAX - 1}{" "}
                units).
              </p>
            : <div>
                {lowStockOnly.map((p) => (
      ...
      {totalAlerts > 0 && (
        <div className='px-5 py-2.5 bg-amber-50/50 border-t border-amber-100/80 text-[11px] text-amber-900/90'>
          ...
          restock or pause ads on out-of-stock items.
        </div>
      )}
    </section>
  );
