...
export function AdminAiBlogDraftSection({ onApply, prefill, compact }: Props) {
  ...
  const [category, setCategory] = useState(
    prefill?.category || "saree-styling",
  );
  ...
  if (!status?.blogEnabled) {
    return (
      <div className='rounded-2xl border border-amber-200 bg-amber-50 p-4 text-xs text-amber-900 leading-relaxed'>
        <strong>Blog AI off.</strong> Backend{" "}
        <code className='text-[10px]'>.env</code> mein{" "}
        <code className='text-[10px]'>GEMINI_API_KEY</code> set karo,{" "}
        <code className='text-[10px]'>GEMINI_MODEL=gemini-2.5-flash</code>, phir
        server restart.
      </div>
    );
  }
  ...
  const run = async (regenerate = false) => {
    ...
    try {
      ...
      if (d.keywordSuggestions?.length)
        setKeywordSuggestions(d.keywordSuggestions);
      setLastDraft(d);
      toast.success(
        regenerate ?
          "New draft generated"
        : "Blog draft ready - pick a title below",
      );
    } catch (e) {
  ...
  return (
    <div
      className={`space-y-4 w-full ${compact ? "" : "rounded-2xl border border-violet-200 bg-gradient-to-br from-violet-50/80 to-white p-4 sm:p-5"}`}
    >
      {!compact && (
        <div className='flex items-start gap-2'>
          <Sparkles className='w-4 h-4 text-violet-600 mt-0.5 shrink-0' />
          <p className='text-xs text-violet-900 leading-relaxed'>
            <strong>AI Journal Writer:</strong> Enter a topic, generate a draft,
      ...
      {keywordSuggestions.length > 0 && (
        <div className='flex flex-wrap gap-1.5'>
          <span className='text-[10px] text-violet-700 font-semibold w-full'>
            RAG keyword ideas:
          </span>
      ...
      <div className='flex flex-wrap gap-2'>
        ...
        <select
          value={targetLength}
          onChange={(e) =>
            setTargetLength(e.target.value as "short" | "medium" | "long")
          }
          className='rounded-lg border border-violet-200 bg-white px-3 py-2 text-xs'
          disabled={loading}
        >
