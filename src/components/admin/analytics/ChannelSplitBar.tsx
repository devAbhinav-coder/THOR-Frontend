...
export default function ChannelSplitBar({
  ...
  if (total === 0 && totalOrders === 0) {
    return (
      <p className='text-xs text-gray-500 py-3 text-center'>
        No paid orders yet - split appears when sales come in.
      </p>
    );
  }
  ...
  const onlinePct =
    total > 0 ?
      (onlineRevenue / total) * 100
    : (onlineCount / Math.max(totalOrders, 1)) * 100;
  const offlinePct =
    total > 0 ?
      (offlineRevenue / total) * 100
    : (offlineCount / Math.max(totalOrders, 1)) * 100;
    ...
    <div className='space-y-3'>
      ...
      <div className='grid grid-cols-3 gap-2'>
        <div className='rounded-lg bg-navy-50/80 border border-navy-100 p-2.5'>
          <p className='text-[9px] font-bold uppercase tracking-wide text-navy-800/80'>
            Online
          </p>
          <p className='text-base font-bold text-navy-950 tabular-nums'>
        ...
        <div className='rounded-lg bg-brand-50/80 border border-brand-100 p-2.5'>
          <p className='text-[9px] font-bold uppercase tracking-wide text-brand-900/80'>
            Offline
          </p>
          <p className='text-base font-bold text-brand-950 tabular-nums'>
            {formatPrice(offlineRevenue)}
          </p>
        ...
        <div className='rounded-lg bg-violet-50/80 border border-violet-100 p-2.5'>
          ...
          <p className='text-base font-bold text-violet-950 tabular-nums'>
            {formatPrice(b2bRevenue)}
          </p>
