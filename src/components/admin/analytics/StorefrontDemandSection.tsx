...
import { adminProductStoreHref } from "@/lib/adminProductStoreLink";
...
export default function StorefrontDemandSection({
  ...
  const stats = useMemo(() => {
    ...
    const listedSold = rows.reduce((s, p) => s + p.sold, 0);
    const totalSold = catalogUnitsSold ?? listedSold;
    const avgConv =
      storefrontConversionPercent ??
      (totalViews > 0 ?
        Math.round((totalSold / totalViews) * 10000) / 100
      : rows.length > 0 ?
        Math.round(
          (rows.reduce((s, p) => s + p.conversionPercent, 0) / rows.length) *
            10,
        ) / 10
      : 0);
    const highConv = rows.filter(
      (p) => p.conversionPercent >= avgConv && p.views >= 5,
    );
    const lowConv = rows.filter(
      (p) => p.conversionPercent < avgConv && p.views >= 10,
    );
    return {
      totalViews,
      totalSold,
    ...
    return (
      ...
      >
        ...
        <p className='text-xs text-gray-500 mt-1'>
          Views appear when customers open product pages (1 per session per
          SKU).
        </p>
      </section>
    );
...
function InsightList({
  ...
  return (
    <div className={`rounded-lg border p-3 ${border}`}>
      <h4 className='text-[10px] font-bold uppercase tracking-wide text-gray-800'>
        {title}
      </h4>
