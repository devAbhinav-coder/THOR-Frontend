      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-2.5">
        {rows.slice(0, 8).map((v) => {
          ...
          return (
            <div 
              key={v._id} 
              className="min-w-0 group p-2.5 rounded-xl transition-all duration-300 hover:bg-[#FAF9F6] border border-transparent hover:border-brand-100/50 cursor-pointer"
            >
              ...
              <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                  <div
                    className="h-full bg-gradient-to-r from-navy-800 to-navy-500 rounded-full transition-all duration-1000 ease-out"
                    style={{ width: `${pct}%` }}
                  />
              </div>
            </div>
