          ...
          <div className='relative flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between'>
            <div>
              ...
              <p className='text-xs sm:text-sm text-navy-200/90 mt-1 max-w-lg'>
                Edit banners, homepage sections, Premium Edit, and site-wide
                content - pick a section below, edit, then press Save.
              </p>
            </div>
          ...
          <div className='flex items-center gap-2 py-2'>
            <div className='flex flex-1 min-w-0 items-center gap-1.5 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden'>
              {STOREFRONT_SECTION_GROUPS.map((group, groupIdx) => {
                const items = STOREFRONT_SECTIONS.filter(
                  (s) => s.group === group,
                );
                return (
                  <div
                  ...
                  >
                    {groupIdx > 0 && (
                      <span
                        className='mx-0.5 h-5 w-px shrink-0 bg-gray-200'
                        aria-hidden
                      />
                    )}
