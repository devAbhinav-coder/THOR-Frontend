...
export default function CouponFormModal({ coupon, onClose, onSave }: Props) {
  ...
  const [formData, setFormData] = useState({
    ...
    eligibilityType:
      coupon?.eligibilityType || ("all" as "all" | "first_order" | "returning"),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (
      ...
      !formData.applicableSubcategoryIds.length
    ) {
    ...
    try {
      ...
      fd.append(
        ...
        JSON.stringify(
          formData.scopeType === "subcategories" ?
            formData.applicableSubcategoryIds
          : [],
        ),
      );
      fd.append(
      ...
      <form onSubmit={handleSubmit} className='space-y-4'>
        ...
        >
          <AdminOfferToggleCards
            ...
            onChange={(v) =>
              setFormData({ ...formData, showOnStorefront: v as boolean })
            }
            options={[
              {
                value: true,
              ...
              },
              {
                ...
                title: "Code only",
                description:
                  "Hidden everywhere - shoppers must type the code manually.",
              },
            ]}
          />
          {!formData.showOnStorefront ?
            <AdminOfferInfoBox tone='amber'>
              This code will <strong>not</strong> appear in popups, cart offers,
              or shop filters. Share it only with people you intend (e.g.
              influencer audience).
            </AdminOfferInfoBox>
          : null}
        </AdminOfferSection>
        ...
        <AdminOfferSection
          ...
          description='Code, discount & display info'
          icon={Tag}
        >
          <div className='grid grid-cols-1 sm:grid-cols-2 gap-3'>
            ...
            <Input
              label={
                formData.discountType === "percentage" ? "Value (%) *"
              ...
              required
              placeholder={
                formData.discountType === "fixed" ? "1150" : undefined
              }
            />
            <Input
              label='Min order (₹)'
        ...
        <AdminOfferSection
          title='Eligibility'
          description='Who can redeem this coupon'
          icon={Users}
        >
          <div className='grid grid-cols-1 sm:grid-cols-2 gap-3'>
            <AdminOfferField label='Who can use'>
              <AdminOfferSelect
                ...
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    eligibilityType: e.target.value as
                      | "all"
                      | "first_order"
                      | "returning",
                  })
            ...
            <Input
              ...
              onChange={(e) =>
                setFormData({
                  ...formData,
                  minCompletedOrders: parseInt(e.target.value) || 0,
                })
              }
            />
          </div>
        ...
        <AdminOfferSection
          title='Applies to'
          description='Which items get the discount'
          icon={ShoppingBag}
        >
          <PromoScopePicker
            scopeType={formData.scopeType}
            onScopeTypeChange={(type) =>
              setFormData({ ...formData, scopeType: type })
            }
            ...
            onToggleSubcategory={(id) =>
              toggleId("applicableSubcategoryIds", id)
            }
