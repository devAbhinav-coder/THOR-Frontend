...
function drawOrderBarcode(canvas: HTMLCanvasElement | null, raw: string) {
  ...
  try {
    ...
    try {
      JsBarcode(canvas, raw.replace(/[^A-Z0-9]/gi, '').slice(0, 24), barcodeOptsOrder());
    } catch {
      /* ignore */
    }
  }
    ...
    <table style={{ width: '100%', ...TABLE_SEP, fontSize: '7px' }}>
      <thead>
        <tr style={{ background: '#f3f4f6' }}>
          <th style={{ ...gridCell(0, 0), padding: '5px 4px', fontWeight: 800, textAlign: 'left', width: '44%' }}>
            Product Name &amp; SKU
          </th>
          <th style={{ ...gridCell(0, 1), padding: '5px 4px', fontWeight: 800, width: '10%', textAlign: 'center' }}>
            Qty.
          </th>
          <th style={{ ...gridCell(0, 2), padding: '5px 4px', fontWeight: 800, width: '20%', textAlign: 'right' }}>
            Price
          </th>
          <th style={{ ...gridCell(0, 3), padding: '5px 4px', fontWeight: 800, width: '26%', textAlign: 'right' }}>
            Total
          </th>
        </tr>
      </thead>
      <tbody>
        {items.map((it, idx) => {
          ...
          return (
            <tr key={idx}>
              <td style={{ ...gridCell(r, 0), padding: '4px 4px', lineHeight: 1.3 }}>
                <span style={{ fontWeight: 600 }}>{it.name}</span>
                {it.variant?.sku ?
                  <span style={{ display: 'block', fontSize: '6px', color: '#444', marginTop: 3, lineHeight: 1.25 }}>
                    SKU:{it.variant.sku}
                  </span>
                : null}
              </td>
              <td style={{ ...gridCell(r, 1), padding: '4px 4px', textAlign: 'center', lineHeight: 1.3 }}>{it.quantity}</td>
              <td style={{ ...gridCell(r, 2), padding: '4px 4px', textAlign: 'right', lineHeight: 1.3 }}>{slipInrItem(it.price)}</td>
              <td style={{ ...gridCell(r, 3), padding: '4px 4px', textAlign: 'right', fontWeight: 600, lineHeight: 1.3 }}>
        ...
        <tr>
          <td colSpan={3} style={{ ...gridCell(totalRow, 0), padding: '5px 4px', textAlign: 'right', fontWeight: 800, lineHeight: 1.3 }}>
            Total
          </td>
          <td style={{ ...gridCell(totalRow, 3), padding: '5px 4px', textAlign: 'right', fontWeight: 900, lineHeight: 1.3 }}>
...
function gridCell(row: number, col: number, extra?: CSSProperties): CSSProperties {
  return {
...
export default function AdminOrderPackingSlip4R({ order, awb }: Props) {
  ...
  const logoSrc =
    typeof window !== 'undefined' ? `${window.location.origin}/logoNew.png` : '/logoNew.png';
  ...
  const downloadPreviewPdf = useCallback(async () => {
    ...
    try {
      ...
      drawAwbBarcode(awbBarRef.current, awbTrim);
      const rawOrder = order.orderNumber?.trim() || order._id?.slice(-12) || 'ORDER';
      ...
      const fullW = Math.ceil(Math.max(el.scrollWidth, el.clientWidth, el.offsetWidth));
      const fullH = Math.ceil(Math.max(el.scrollHeight, el.clientHeight, el.offsetHeight));
      ...
      const raw =
        order.orderNumber?.trim().replace(/[^\w.-]+/g, '_') || order._id?.slice(-8) || 'order';
...
<script>window.addEventListener("load",function(){setTimeout(function(){window.print();},450);});</script>
  ...
  const cityStateBold = a?.city && a?.state ? `${a.city} (${a.state})` : a?.city || a?.state || '—';
      ...
      >
        ...
        <table style={{ width: '100%', ...TABLE_SEP, tableLayout: 'fixed' }}>
          <tbody>
            {/* Header logos */}
            <tr>
              <td style={{ ...outerCell(0, 0), padding: '4px 5px', width: '52%' }}>
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={logoSrc} alt="" style={{ height: 22, width: 'auto', objectFit: 'contain', display: 'block' }} />
              </td>
              <td style={{ ...outerCell(0, 1), padding: '4px 5px', textAlign: 'right', verticalAlign: 'middle' }}>
                <span style={{ fontWeight: 900, fontSize: '11px', letterSpacing: '-0.04em' }}>
            ...
            <tr>
              <td colSpan={2} style={{ ...outerCell(1, 0), padding: '4px 5px 6px' }}>
                <div style={{ fontWeight: 700, fontSize: '7.5px', marginBottom: 3, lineHeight: 1.25 }}>AWB# {awbTrim}</div>
                <div style={{ textAlign: 'center', padding: '2px 0 6px', lineHeight: 1.15 }}>
                  <canvas
                    ref={awbBarRef}
                    data-barcode="awb"
                    style={{
                ...
                <table style={{ width: '100%', marginTop: 2, fontSize: '7.5px', tableLayout: 'fixed', borderCollapse: 'separate', borderSpacing: 0 }}>
                  <tbody>
                    <tr>
                      <td style={{ fontWeight: 700, width: '26%', verticalAlign: 'bottom', padding: '4px 4px 2px 0', lineHeight: 1.25 }}>{pin}</td>
                      <td style={{ fontWeight: 700, textAlign: 'center', verticalAlign: 'bottom', padding: '4px 4px 2px', lineHeight: 1.25 }}>
                        AWB# {awbTrim}
                      </td>
                      <td style={{ fontWeight: 800, fontSize: '8px', textAlign: 'right', width: '26%', verticalAlign: 'bottom', padding: '4px 0 2px 4px', lineHeight: 1.25 }}>
                        {route}
                      </td>
                    </tr>
                  </tbody>
            ...
            <tr>
              <td style={{ ...outerCell(2, 0), padding: '6px 6px', width: '58%' }}>
                <div style={{ fontWeight: 800, fontSize: '7px', marginBottom: 3, lineHeight: 1.25 }}>Shipping address:</div>
                <div style={{ marginBottom: 3, lineHeight: 1.25 }}>
                  <span style={{ fontWeight: 800, fontSize: '8px' }}>{recipient}</span>
                </div>
                <div style={{ fontSize: '6.5px', color: '#222', marginBottom: 3, lineHeight: 1.3 }}>
                  Phone: {phone || '—'}
                </div>
                <div style={{ fontSize: '7px', marginBottom: 3, lineHeight: 1.3 }}>{addrBody}</div>
                <div style={{ fontWeight: 800, fontSize: '7px', marginBottom: 3, lineHeight: 1.25 }}>{cityStateBold}</div>
                <div style={{ fontWeight: 900, fontSize: '7.5px', lineHeight: 1.25 }}>PIN — {pin}</div>
              </td>
              <td style={{ ...outerCell(2, 1), padding: '6px 6px', width: '42%' }}>
                <div style={{ fontWeight: 900, fontSize: '8px', lineHeight: 1.25 }}>{paymentKind}</div>
                <div style={{ fontWeight: 800, fontSize: '7.5px', marginTop: 3, color: '#111', lineHeight: 1.25 }}>{shipMode}</div>
                <div style={{ fontWeight: 900, fontSize: '13px', marginTop: 5, letterSpacing: '-0.02em', lineHeight: 1.2 }}>
                  {slipInr(order.total)}
                </div>
                <div style={{ borderTop: '1px solid #bbb', margin: '6px 0 4px' }} />
                <div style={{ fontSize: '6.5px', color: '#111', fontWeight: 700, lineHeight: 1.3, paddingBottom: 2 }}>
            ...
            <tr>
              <td style={{ ...outerCell(3, 0), padding: '6px 6px' }}>
                <div style={{ fontSize: '6.5px', fontWeight: 700, color: '#333', marginBottom: 3, lineHeight: 1.25 }}>Seller</div>
                <div style={{ fontWeight: 900, fontSize: '9px', lineHeight: 1.25, marginBottom: 4 }}>{SELLER_NAME}</div>
                <div style={{ fontSize: '6.5px', lineHeight: 1.35, color: '#111' }}>{SELLER_ADDRESS}</div>
              </td>
              <td style={{ ...outerCell(3, 1), padding: '6px 6px' }}>
                <div style={{ fontSize: '6.5px', color: '#555', marginBottom: 3, lineHeight: 1.25 }}>Order ID</div>
                <div
                  style={{
                ...
                <div style={{ textAlign: 'center', padding: '2px 0', lineHeight: 1.15 }}>
                  <canvas
                    ref={orderBarRef}
                    data-barcode="order"
                    style={{
            ...
            {/* Products */}
            <tr>
              <td colSpan={2} style={{ ...outerCell(4, 0), padding: '8px 4px 4px', verticalAlign: 'top' }}>
                <ProductLinesTable order={order} gridCell={gridCell} slipInrItem={slipInrItem} />
              </td>
            </tr>

            {/* Thin spacer before return block */}
            <tr>
              <td colSpan={2} style={{ ...outerCell(5, 0), padding: '4px 0', background: '#fff', borderBottom: 'none' }}>
            ...
            <tr>
              ...
              >
                <div style={{ fontSize: '6.5px', lineHeight: 1.45, color: '#111', fontWeight: 800 }}>Return address</div>
                <div style={{ fontSize: '6.5px', lineHeight: 1.4, color: '#111', marginTop: 3 }}>{RETURN_ADDRESS_TEXT}</div>
              </td>
            </tr>
            <tr>
              <td
