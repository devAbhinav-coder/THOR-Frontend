          ...
          return (
            <div key={id} className='bg-white group'>
              ...
              >
                ...
                <span className='flex items-center justify-center w-6 h-6 rounded-full bg-brand-50 text-brand-700 text-[11px] font-bold tabular-nums shrink-0'>
                  {idx + 1}
                </span>
                ...
                <div className='min-w-0 flex-1'>
                  ...
                  <p className='text-[11px] text-gray-500 truncate'>
                    {String(p.category || "").toLowerCase() === "premium" ?
                      "Premium Edit"
                    : p.category}
                  </p>
                </div>
                <div className='hidden sm:grid sm:grid-cols-4 gap-3 text-right shrink-0'>
                  ...
                  <div>
                    ...
                    <p className='text-xs font-bold tabular-nums text-brand-700'>
                ...
                <div className='sm:hidden text-right shrink-0 min-w-[5.5rem]'>
                  ...
                  <p className='text-[10px] text-brand-600 uppercase mt-1'>
                    Profit
                  </p>
              ...
              {open && (
                <div className='px-4 pb-4 pt-0 bg-gradient-to-b from-brand-50/20 to-white border-t border-brand-50/50'>
                  <div className='grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 py-3'>
                    ...
                    <Detail
                      label='Gross profit'
                      value={formatPrice(p.profit)}
                      highlight
                    />
                    <Detail
                      label='Avg sell price'
                      value={formatPrice(p.avgSellPrice)}
                    />
                    <Detail
                      label='Avg unit cost'
                      value={formatPrice(p.avgUnitCost)}
                    />
                  </div>
                  {missingCost && (
                    <p className='flex items-center gap-1.5 text-[11px] text-amber-800 bg-amber-50 border border-amber-100 rounded-lg px-2.5 py-2 mb-2'>
                      ...
                      {p.linesMissingCost} of {p.orderLines} lines missing cost
                      - set variant cost in Inventory.
                    </p>
                  )}
                  <Link
                    href={`/admin/products`}
