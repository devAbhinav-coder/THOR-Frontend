...
export default function ProfitHeroStrip({
  ...
  const costCoverage =
    totalLines > 0 ?
      Math.round(((totalLines - missingCost) / totalLines) * 100)
    : 100;
      ...
      <div className='flex flex-col sm:flex-row sm:items-end sm:justify-between gap-2'>
        <div>
          ...
          <p className='text-xs text-gray-500 mt-0.5 max-w-xl'>
            Profit = paid line revenue − variant cost (SKU match). Uses catalog
            cost today - add cost on variants in Inventory for accuracy.
          </p>
        </div>
      ...
      <div className='grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3'>
        {cards.map((c) => (
          ...
          >
            ...
            <p
              className={cn(
                "text-2xl font-bold font-serif tabular-nums mt-2 tracking-tight",
                c.valueClass,
              )}
            >
              {c.value}
            </p>
            <p className={cn("text-[11px] mt-1.5 leading-snug", c.subClass)}>
              {c.sub}
            </p>
          </article>
        ))}
