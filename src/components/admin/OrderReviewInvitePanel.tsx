...
export default function OrderReviewInvitePanel({ orderId, className }: Props) {
  ...
  const sendEmail = async () => {
    ...
    try {
      ...
      toast.error(
        (err as { message?: string })?.message || "Could not send email",
      );
    } finally {
      setEmailing(false);
    }
  ...
  const sendWhatsApp = async () => {
    ...
    try {
      ...
      toast.error(
        (err as { message?: string })?.message || "Could not send WhatsApp",
      );
    } finally {
  ...
  const downloadQr = () => {
    ...
    downloadDataUrl(
      src,
      `THOR-review-QR-${data?.order.orderNumber || orderId}.png`,
    );
  ...
  const isBrandedCard = Boolean(
    brandedQr && brandedQr !== data?.invite.qrDataUrl,
  );
            ...
            <div className='px-4 py-4 space-y-3.5'>
              ...
              <p className='flex flex-wrap items-center gap-x-2 gap-y-1 text-[11px] text-gray-400'>
                <span className='inline-flex items-center gap-1'>
                  ...
                  Exp{" "}
                  {new Date(data.invite.expiresAt).toLocaleDateString("en-IN")}
                </span>
                <span>·</span>
                <span>
                  Reviewed {data.invite.reviewedCount}/
                  {data.invite.productCount}
                </span>
              ...
              <div className='grid grid-cols-2 gap-2'>
                ...
                <Button
                  ...
                  loading={whatsApping}
                  disabled={
                    !data.order.hasWhatsAppPhone && !data.order.customerPhone
                  }
