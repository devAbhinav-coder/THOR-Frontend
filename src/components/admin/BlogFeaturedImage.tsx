  ...
  return (
    <div className='rounded-2xl border-2 border-dashed border-amber-200 bg-amber-50/30 p-5'>
      ...
      {coverUrl ?
        <div className='flex flex-col sm:flex-row gap-4'>
          ...
          <div className='flex-1 text-sm text-gray-600 space-y-2'>
            <p>
              This image appears in the <strong>hero banner</strong> at the top
              of the published article. It is not inserted into the story body.
            </p>
            <p className='text-xs text-gray-500'>
              To add images within the article, use <strong>Add Media</strong>{" "}
              in the content editor or select <strong>Insert in story</strong>{" "}
      ...
      : <div className='flex items-center gap-3 py-6 px-4 rounded-xl bg-white border border-amber-100 text-gray-500'>
          ...
          <p className='text-sm'>
            Upload your first image - it will automatically become the featured
            cover. You can change it anytime from the gallery or media library.
          </p>
        </div>
      }
...
function setCoverOnRows(rows: GalleryRow[], index: number): GalleryRow[] {
  return rows.map((row, i) => ({
    ...row,
    placement:
      i === index ? ("cover" as const)
      : row.placement === "cover" ? ("article" as const)
      : row.placement,
