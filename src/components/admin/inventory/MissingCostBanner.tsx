...
export default function MissingCostBanner({
  ...
  if (
    missingSkus <= 0 &&
    !(periodLinesMissingCost && periodLinesMissingCost > 0)
  ) {
    return null;
  }
      ...
      <div className='flex items-start gap-2.5'>
        ...
        <div>
          ...
          <p className='text-xs text-amber-800/90 mt-0.5 leading-relaxed'>
            {missingSkus > 0 && (
              <>
                <strong>{missingSkus}</strong> of {totalSkus} SKUs ({pct}%) have
                no cost set - margin &amp; profit may be overstated.
              </>
            )}
            {periodLinesMissingCost != null &&
              ...
              periodLinesMissingCost > 0 && (
                <>
                  {missingSkus > 0 ? " · " : ""}
                  <strong>{periodLinesMissingCost}</strong> of{" "}
                  {periodOrderLines} order lines in this period lack cost.
                </>
              )}{" "}
