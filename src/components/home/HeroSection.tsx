...
const fallbackSlides: HeroSlide[] = [
  {
    ...
    description: 'Discover our handwoven Banarasi and Kanjeevaram silk sarees — timeless beauty for every occasion.',
    ctaText: 'Shop Sarees',
  ...
  },
  {
    ...
    description: 'Make your special day unforgettable with our exquisite bridal lehengas, crafted for the modern bride.',
  ...
  },
  {
    ...
    description: 'Effortlessly stylish kurtis and salwar suits for every mood and every moment of your day.',
    ctaText: 'Shop Now',
  ...
  useEffect(() => {
    storefrontApi.getSettings()
      .then((res) => {
        ...
        const activeSlides = incoming.filter((s: HeroSlide) => s.isActive !== false && s.image && s.title);
    ...
    <section className="relative h-[88vh] min-h-[520px] overflow-hidden bg-navy-950">
      ...
      <div className="relative h-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center">
        <div className="max-w-xl animate-fadeIn">
          ...
          {slide.subtitle && (
            <p className="text-gold-400 font-medium mb-2 text-base tracking-wide">{slide.subtitle}</p>
          )}
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-serif font-bold text-white leading-tight mb-5 drop-shadow-lg">
            {slide.title}
          </h1>
          {slide.description && (
            <p className="text-white/70 text-lg mb-8 leading-relaxed">{slide.description}</p>
          )}
          <div className="flex flex-wrap gap-3">
            <Button asChild size="xl" className="bg-brand-600 hover:bg-brand-700 text-white shadow-lg shadow-brand-900/40">
            ...
            >
              <Link href={slide.secondaryCtaLink || '/shop'}>{slide.secondaryCtaText || 'View All'}</Link>
            </Button>
          </div>
        </div>
      </div>
