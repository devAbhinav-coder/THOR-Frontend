    ...
    <div className='flex flex-col gap-account-stack-lg pb-8'>
      ...
      <header>
        <nav className='flex items-center gap-2 text-[11px] font-semibold uppercase tracking-widest text-account-outline mb-4'>
          <Link
            ...
            className='hover:text-account-secondary transition-colors'
          >
            Orders
          </Link>
          <ChevronRight className='h-3.5 w-3.5' />
          <span className='text-account-on-surface-variant'>
            #{order.orderNumber}
          </span>
        </nav>
              ...
              {order.items.map((item: OrderItem, i: number) => {
                ...
                return (
                  <div
                    ...
                    className={cn(
                      i > 0 &&
                        "pt-8 border-t border-account-outline-variant/20",
                    )}
                  >
                    <div className='flex flex-col sm:flex-row gap-6'>
                      <div className='relative w-full sm:w-36 h-48 sm:h-52 shrink-0 bg-account-surface-container overflow-hidden'>
                        {(
                          productSlug &&
                          isUsableOrderLineImage(item.image, {
                        ...
                        : <OrderLineThumbnail
                            ...
                            name={item.name}
                            isOfflineManual={
                              item.isOfflineManual ||
                              item.slug === "offline-manual-item"
                            }
                            className='h-full w-full border-0 rounded-none'
                            sizes='144px'
                          />
                      ...
                      <div className='flex-1 min-w-0'>
                        ...
                        : <h3 className='font-serif text-xl md:text-2xl text-account-primary italic'>
                            {item.name}
                          </h3>
                        }
                        {item.variant.sku && (
                        ...
                        <div className='flex flex-wrap gap-8 mt-6'>
                          <div>
                            <p className='text-[10px] font-semibold uppercase tracking-widest text-account-outline'>
                              Quantity
                            </p>
                    ...
                    {isReviewOpen && canReview && (
                      <OrderReviewForm
                        ...
                        onUpdate={(field, value) =>
                          onUpdateReview(productId, field, value)
                        }
                        onImagesChange={(e) =>
                          onReviewImagesChange(productId, e)
                        }
                        onRemoveImage={(idx) =>
                          onRemoveReviewImage(productId, idx)
                        }
        ...
        <aside className='lg:col-span-4'>
          <div className='bg-account-surface-container-lowest border border-account-outline-variant/30 p-6 md:p-8 lg:sticky lg:top-24'>
              ...
              <div className='flex justify-between text-account-on-surface-variant'>
                ...
                <span>
                  {order.shippingCharge === 0 ?
                    <span className='text-account-secondary font-medium'>
                      Complimentary
                    </span>
            ...
            <div className='flex justify-between items-baseline mt-6 pt-6 border-t border-account-outline-variant/30'>
              <span className='text-[11px] font-semibold uppercase tracking-widest text-account-outline'>
                Total
              </span>
              <span className='font-serif text-2xl text-account-primary'>
            ...
            <div className='mt-6 bg-account-secondary-container/30 border border-account-secondary-container/50 p-4'>
              <div className='flex gap-2'>
                ...
                <p className='text-xs text-account-on-surface-variant leading-relaxed'>
                  Each piece is crafted with care. Our team is available for
                  preservation guidance after delivery.
                </p>
              </div>
            </div>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 gap-account-gutter'>
        <section className='bg-account-surface-container-lowest border border-account-outline-variant/30 p-6 md:p-8'>
          ...
          <p className='font-serif text-lg text-account-primary mb-2'>
          ...
          <p className='text-sm text-account-primary mt-3'>
        ...
        <section className='bg-account-surface-container-lowest border border-account-outline-variant/30 p-6 md:p-8'>
          ...
          <p className='font-serif text-lg text-account-primary'>
            {paymentMethodLabel(order.paymentMethod)}
          </p>
          <p className='text-sm text-account-on-surface-variant mt-2 capitalize'>
            {order.paymentStatus}
          </p>
          {order.razorpayPaymentId && (
            <p className='text-xs text-account-outline mt-2 font-mono'>
              Ref: …{order.razorpayPaymentId.slice(-8)}
            </p>
          )}
