...
function exportCsv(bySupplier: SupplierRow[], year: number) {
  ...
  a.href = url;
  a.download = `gst-purchase-summary-${year}.csv`;
  a.click();
...
export default function GstSummaryTab() {
  ...
  const {
    ...
    queryFn: async () => {
      try {
        const params: { year: number; month?: string; quarter?: string } = {
          year,
        };
        if (month !== "all") {
          params.month = month;
        ...
        return {
          bySupplier: ((res.data as { bySupplier?: SupplierRow[] })
            .bySupplier ?? []) as SupplierRow[],
          monthly: ((res.data as { monthly?: MonthlyRow[] }).monthly ??
            []) as MonthlyRow[],
          totals: ((res.data as { totals?: Totals }).totals ??
            null) as Totals | null,
        };
      } catch (err) {
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => {
                ...
                if (e.target.value !== "all") setMonth("all");
              }}
              className='h-9 px-3 rounded-xl border border-gray-200 text-sm focus:ring-2 focus:ring-brand-500 focus:outline-none'
            >
        ...
        <div className='grid grid-cols-2 sm:grid-cols-6 gap-3'>
          {[
            ...
            >
              ...
              <p className={`text-base font-bold mt-0.5 ${c.color}`}>
                {fmt(c.value)}
              </p>
            </div>
          ))}
