        ...
        <div className='relative'>
          ...
          {line.product && (
            <p className='text-[9px] text-emerald-600 font-bold mt-1 flex items-center gap-1'>
              ...
              <button
                type='button'
                onClick={() => onUpdate("product", undefined)}
      ...
      <div className='grid grid-cols-3 sm:grid-cols-6 gap-3 items-end pt-2 border-t border-gray-100'>
        ...
        <div className='text-xs'>
          ...
          <p className='font-bold text-emerald-600'>
            ₹{(line.cgst + line.sgst + line.igst).toFixed(2)}
          </p>
        </div>
        <div className='flex items-center justify-between bg-white px-3 py-1.5 rounded-lg border border-brand-100'>
          ...
          {onRemove && (
            <button
              type='button'
              onClick={onRemove}
              className='p-1.5 hover:bg-red-50 text-gray-300 hover:text-red-600 rounded-lg'
            >
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 text-xs font-semibold px-2 py-0.5 rounded-full ${map[status] ?? "bg-gray-100 text-gray-700"}`}
    >
    ...
    <ModalShell title='New Purchase Invoice' onClose={onClose}>
      ...
      <label className='flex items-center gap-2 text-sm text-gray-700 cursor-pointer'>
        <input
          ...
          onChange={(e) =>
            setForm((p) => ({ ...p, updateCostPrice: e.target.checked }))
          }
  ...
  const handle = async () => {
    ...
    try {
      await inventoryApi.updatePurchaseInvoice(invoice._id, {
        ...
        paidAmount:
          form.paymentStatus === "paid" ? invoice.grandTotal : form.paidAmount,
      });
  ...
  return (
    <ModalShell title={`Edit · ${invoice.invoiceNumber}`} onClose={onClose}>
      <p className='text-xs text-amber-700 bg-amber-50 border border-amber-100 rounded-lg px-3 py-2'>
        Line items & GST amounts cannot be changed after creation (stock already
        posted). Edit supplier, payment & notes only.
      </p>
      <InvoiceHeaderFields form={form} setForm={setForm} />
      <ModalFooter
        onClose={onClose}
...
function ViewModal({
  ...
  const print = () => {
    ...
    w.document
      .write(`<html><head><title>${invoice?.invoiceNumber ?? "Invoice"}</title>
    ...
    <div className='grid grid-cols-1 sm:grid-cols-2 gap-4'>
            ...
            {f.key === "invoiceNumber" && (
              <button
                ...
                onClick={() =>
                  setForm(
                    (p) =>
                      ({ ...p, invoiceNumber: generateInvoiceNumber() }) as T,
                  )
                }
      ...
      <div>
        ...
        <select
          ...
          onChange={(e) =>
            setForm((p) => ({ ...p, supplyType: e.target.value }) as T)
          }
      ...
      <div>
        ...
        <select
          ...
          onChange={(e) =>
            setForm((p) => ({ ...p, paymentStatus: e.target.value }) as T)
          }
          className='w-full h-10 px-3 rounded-xl border border-gray-200 text-sm'
        >
      ...
      {(form as Record<string, unknown>).paymentStatus === "partial" && (
        <div>
          ...
          <input
            ...
            onChange={(e) =>
              setForm(
                (p) => ({ ...p, paidAmount: Number(e.target.value) }) as T,
              )
            }
            className='w-full h-10 px-3 rounded-xl border border-gray-200 text-sm'
          />
      ...
      <div className='sm:col-span-2'>
        ...
        <input
          ...
          onChange={(e) =>
            setForm((p) => ({ ...p, notes: e.target.value }) as T)
          }
...
function LineItemsEditor({
  ...
  useEffect(() => {
    setLines((prev) =>
      prev.map((l) => ({
        ...l,
        ...calcLine(l.quantity, l.unitCost, l.gstRate, supplyType),
      })),
    );
  }, [supplyType, setLines]);
...
export default function PurchaseInvoicesTab() {
  ...
  const handleVoid = (inv: PurchaseInvoice) => {
    if (
      !confirm(
        `Void invoice ${inv.invoiceNumber}? This cannot be undone. Stock is NOT reversed automatically.`,
      )
    )
      return;
        ...
        <div className='grid grid-cols-2 sm:grid-cols-4 gap-3'>
          {[
            ...
            },
            {
              ...
              value: formatPrice(summary.outstandingPayable),
              icon: AlertCircle,
              color: "text-red-600",
            },
          ].map((c) => (
            ...
            >
              ...
              {c.label === "Outstanding Payable" &&
                summary.unpaidCount + summary.partialCount > 0 && (
                  <p className='text-[10px] text-gray-500 mt-0.5'>
                    {summary.unpaidCount} unpaid · {summary.partialCount}{" "}
                    partial
                  </p>
                )}
            </div>
      ...
      <div className='bg-white rounded-xl border border-gray-100 shadow-sm p-4'>
        <div className='flex flex-col lg:flex-row gap-3'>
          ...
          <button
            ...
            className='p-2 rounded-xl border border-gray-200 hover:bg-gray-50 h-10'
          >
            <RefreshCw
              className={`h-4 w-4 text-gray-500 ${loading || isFetching ? "animate-spin" : ""}`}
            />
          </button>
          <Button
            variant='brand'
            size='sm'
            className='rounded-xl h-10 ml-auto'
        ...
        {totalPages > 1 && (
          <div className='flex items-center justify-between px-4 py-3 border-t border-gray-100'>
            <p className='text-xs text-gray-500'>
              {total} invoice{total !== 1 ? "s" : ""} · Page {page}/{totalPages}
            </p>
