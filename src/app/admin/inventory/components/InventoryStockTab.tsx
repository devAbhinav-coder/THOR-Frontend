...
function StockBadge({ stock }: { stock: number }) {
  ...
  if (stock < LOW_STOCK_ALERT_EXCLUSIVE_MAX)
    return (
      ...
      <div className='bg-white rounded-2xl shadow-xl w-full max-w-md p-6 space-y-4'>
        ...
        <div className='bg-gray-50 rounded-xl p-3 text-sm'>
          ...
          <p className='text-gray-500 text-xs mt-0.5'>
            SKU: {variant.sku} · Current stock: {variant.stock}
          </p>
        </div>
        <div className='space-y-4'>
          ...
          <div className='grid grid-cols-2 gap-4'>
            <div>
              <label className='text-[10px] font-bold text-gray-400 uppercase mb-1 block text-brand-600'>
                Unit Cost ₹
              </label>
              <input
                type='number'
            ...
            <div>
              <label className='text-[10px] font-bold text-gray-400 uppercase mb-1 block text-navy-600'>
                Selling Price ₹
              </label>
              <input
                type='number'
              ...
              return (
                <tr key={v.sku} className='hover:bg-gray-50/80'>
                  ...
                  <td className='px-4 py-2.5 text-right font-semibold text-gray-900'>
                    {formatPrice(sell)}
                    {sellOverride && (
                      <p className='text-[9px] font-normal text-brand-600'>
                        variant price
                      </p>
                    )}
                  </td>
                  ...
                  {!periodView ?
                    <td className='px-4 py-2.5 text-right'>
                      {varMargin != null ?
                        <span
                          className={cn("font-bold", marginColor(varMargin))}
                        >
                          {varMargin}%
                        </span>
            ...
            >
              <div className='inline-flex flex-col items-end cursor-help'>
                <p className='font-bold text-brand-800 inline-flex items-center gap-1'>
                  {formatPrice(
                    product.grossRevenue ?? product.estimatedRevenue ?? 0,
                  )}
                  {infoIcon}
                </p>
            ...
            >
              <div className='inline-flex flex-col items-end cursor-help'>
                <p className='font-semibold text-gray-600 inline-flex items-center gap-1'>
                  {formatPrice(
                    product.grossCostOfSales ?? product.estimatedCost ?? 0,
                  )}
                  {infoIcon}
                </p>
          ...
          {hasSales ?
            ...
            >
              <div className='inline-flex flex-col items-end cursor-help'>
                ...
                >
                  {formatPrice(
                    product.grossProfit ?? product.estimatedProfit ?? 0,
                  )}
                  {infoIcon}
                </p>
            ...
            <MetricTooltip
              ...
              lines={[
                ...
                },
                {
                  label: "Avg cost",
                  value:
                    product.avgCost ? formatPrice(product.avgCost) : "Not set",
                },
              ...
              <div className='inline-flex flex-col items-end cursor-help'>
                ...
                {product.avgCost != null && product.avgCost > 0 && (
                  <p className='text-[9px] text-gray-400 mt-0.5'>
                    avg cost {formatPrice(product.avgCost)}
                  </p>
                )}
              </div>
            </MetricTooltip>
...
export default function InventoryStockTab() {
  ...
  const {
    ...
    queryFn: async () => {
      try {
        ...
        const opex = (
          opexRes.data as {
            summary?: { yearTotal?: number; monthToDateTotal?: number };
          }
        ).summary;
        return {
          products:
            (res.data as { products?: InventoryProduct[] }).products ?? [],
          summary:
            (res.data as { summary?: InventoryBusinessSummaryData }).summary ??
            null,
          operatingCosts:
            opex ?
              {
                yearTotal: opex.yearTotal ?? 0,
                monthToDateTotal: opex.monthToDateTotal ?? 0,
              }
            : null,
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const rows =
        (res.data as { rows?: Record<string, unknown>[] }).rows ?? [];
      downloadCsv(
  ...
  return (
    <div className='space-y-5'>
      ...
      {reorderItems.length > 0 && (
        <ReorderSuggestionsPanel items={reorderItems} />
      )}
