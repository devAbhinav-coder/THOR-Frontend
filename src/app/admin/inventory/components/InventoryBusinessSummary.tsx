...
function FlowMini({
  ...
  return (
    <div
      className={cn(
        "rounded-xl border px-3 py-2 min-w-[7rem] flex-1",
        styles[tone],
      )}
    >
...
export default function InventoryBusinessSummary({
  ...
  const grossRevenue =
    summary.totalGrossRevenue ?? summary.totalEstimatedRevenue ?? 0;
  const grossProfit =
    summary.totalGrossProfit ?? summary.totalEstimatedProfit ?? 0;
    ...
    <div className='space-y-3'>
        ...
        <div>
          ...
          <p className='text-xs text-gray-500 max-w-xl'>
            {summary.period && summary.period !== "lifetime" ?
              <>
                Sales &amp; profit for <strong>{summary.periodLabel}</strong>{" "}
                from paid orders (website + offline/POS). Stock values are
                current snapshot.
              </>
            : <>
                Sold / revenue / profit from catalog{" "}
                <strong>soldCount × MRP/cost</strong> (per SKU). Hover any
                metric for the full formula.
              </>
            }{" "}
      ...
      <div className='grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3 overflow-visible'>
        ...
        <PrimaryKpi
          ...
          sublabel={
            isPeriod ?
              "Actual paid line totals"
            : "Units sold × avg MRP (catalog)"
          }
        ...
        <PrimaryKpi
          ...
          sublabel={
            isPeriod ?
              "Revenue − COGS (orders)"
            : "Est. revenue − cost of sold units"
          }
      ...
      {sold > 0 && (
        <div className='rounded-[1.25rem] border border-gray-200/80 bg-white/80 backdrop-blur-sm p-4 shadow-sm overflow-visible'>
          ...
          <div className='flex flex-wrap items-center gap-2'>
            <FlowMini
              label={isPeriod ? "Revenue" : "Est. revenue"}
              value={formatPrice(grossRevenue)}
              tone='base'
            />
          ...
          {!isPeriod && (
            <p className='text-[10px] text-gray-400 mt-2.5 leading-relaxed'>
              <strong>Est.</strong> = catalog estimate from units already sold ×
              list price/cost - not a future forecast. Actual cash is on the{" "}
              <Link
                href='/admin/revenue'
                className='text-brand-600 font-semibold hover:underline'
              >
        ...
        <div className='flex flex-wrap divide-x divide-gray-100'>
          ...
          <SecondaryStat
            ...
            tooltip={
              <MetricTooltip
                title='Stock @ cost'
                lines={tips.stockAtCost}
                note={INVENTORY_METRIC_NOTES.variantStock}
              >
                {infoIcon}
              </MetricTooltip>
            }
          ...
          <SecondaryStat
            ...
            tooltip={
              <MetricTooltip
                title='Stock @ MRP'
                lines={tips.stockAtMrp}
                note={INVENTORY_METRIC_NOTES.effectiveSellPrice}
              >
                {infoIcon}
              </MetricTooltip>
            }
          ...
          <SecondaryStat
            ...
            tooltip={
              <MetricTooltip
                title='COGS on sold units'
                lines={tips.grossProfit.slice(1, 2)}
              >
                {infoIcon}
              </MetricTooltip>
            }
