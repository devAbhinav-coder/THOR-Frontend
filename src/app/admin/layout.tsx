...
function AdminAvatar({
  ...
  const initialsTextClass = sizeClass.includes('h-9') ? 'text-[10px]' : 'text-sm';
  if (avatarUrl) {
    return (
      <div
      ...
      <div className="lg:hidden shrink-0 z-40 border-b border-white/[0.08] bg-[#0c0f14] text-white shadow-[0_8px_32px_-12px_rgba(0,0,0,0.55)] sticky top-0">
        <div className="h-[3.25rem] px-3 flex items-center justify-between gap-2">
          <div className="flex min-w-0 flex-1 items-center gap-2">
            ...
            <AdminAvatar name={user?.name} avatarUrl={user?.avatar} sizeClass="h-9 w-9" />
            <div className="min-w-0">
              ...
              <p className="truncate text-[10px] text-slate-300">{user?.name}</p>
            </div>
          </div>
        ...
        {isMenuOpen && (
          ...
          >
            ...
            >
              <Store className="h-4 w-4 shrink-0 text-amber-100" strokeWidth={2} />
              {SHOP_SITE_CTA}
              <ExternalLink className="h-3.5 w-3.5 shrink-0 text-amber-100/90" strokeWidth={2} />
            </Link>
        ...
        <div className="relative z-[1] border-b border-white/[0.06] px-2 pb-2 pt-2">
          ...
          */}
          <div className="rounded-xl bg-white/[0.04] p-2 ring-1 ring-white/[0.08] backdrop-blur-sm">
            ...
            >
              <Store className="h-3.5 w-3.5 shrink-0 text-amber-50" strokeWidth={2} />
              {!sidebarCollapsed && (
                <>
                  <span className="min-w-0 flex-1 truncate text-left">{SHOP_SITE_CTA}</span>
                  <ExternalLink className="h-3 w-3 shrink-0 text-amber-100/90" strokeWidth={2} />
                </>
              )}
              {sidebarCollapsed && (
          ...
          >
            {sidebarCollapsed ? (
              ...
              <>
                ...
                <span className="flex min-h-9 min-w-0 flex-1 items-center truncate">Sign out</span>
              </>
            )}
          </button>
        </div>
