...
export default function AdminRevenuePage() {
  ...
  const {
    ...
    reload: reloadPeriod,
  } = useRevenuePeriod(period, year, undefined, channel);
  ...
  const years = useMemo(
    () =>
      analytics ?
        availableYearsFromAnalytics(analytics)
  ...
  const loadInventory = useCallback(async () => {
    try {
      ...
      setInvValuation(
        (invRes.data as { overall?: InventoryValuationOverall }).overall ??
          null,
      );
      const sum = (opexRes.data as { summary?: { yearTotal?: number } })
        .summary;
  ...
  if (!isLoading && loadError && !analytics) {
    return (
      <div className='p-6 xl:p-8 max-w-3xl mx-auto min-h-[calc(100dvh-4rem)] bg-[#FAF9F6]'>
        <AdminPageHeader
          title='Revenue'
          description='Financial and profit intelligence.'
        />
        <div className='mt-8'>
  ...
  const feesRetained =
    periodOverview?.nonRefundableFeesRetained ??
    overview.nonRefundableFeesRetained ??
    0;
  ...
  const fin = buildFinancialSnapshot(
    ...
    refunded,
    feesRetained,
    period === "year" ? yearOpex
    : period === "lifetime" ? yearOpex
    : 0,
    { usePeriodFields: !!periodOverview },
  );

  const couponTotal =
    periodOverview?.couponDiscountTotal ?? overview.couponDiscountTotal ?? 0;
  const couponOrders =
  ...
  const shippingTotal =
    periodOverview?.shippingCollected ?? overview.shippingCollected ?? 0;
  const codTotal =
    periodOverview?.codFeeCollected ?? overview.codFeeCollected ?? 0;
  ...
  const profitByMonth =
    periodData?.profitByMonth ?? analytics.profitByMonth ?? [];
  const refundsByMonth =
    periodData?.refundsByMonth ?? analytics.refundsByMonth ?? [];
  const productsByProfit =
    periodData?.topProductsByProfit ?? analytics.topProductsByProfit ?? [];
  const categoryProfit =
    periodData?.categoryProfit ?? analytics.categoryProfit ?? [];

  const chartSubtitle =
    period === "month" ?
      "Last 12 months · hover for gross, net, product revenue & profit"
    : period === "year" ? `${year} monthly · gross vs net vs product revenue`
    : "Lifetime trend (recent months) · gross vs net vs profit";

  const refundRatePct =
    (periodOverview?.orders ?? overview.totalOrders) > 0 ?
      ((periodOverview?.refundedOrdersCount ??
        overview.refundedOrdersCount ??
        0) /
  ...
  const opexForPeriod =
    period === "year" && year === new Date().getFullYear() ? yearOpex : 0;
        ...
        <div className='flex flex-col gap-4'>
          <div className='flex flex-col md:flex-row gap-4 items-stretch md:items-center justify-between'>
            ...
            <div className='rounded-xl border border-gray-200/80 bg-white/80 px-4 py-2.5 shadow-sm flex items-center gap-2 text-sm text-gray-600 backdrop-blur-sm shrink-0'>
              ...
              <span>
                As of{" "}
                <time dateTime={new Date().toISOString()}>{asOfLabel}</time>
              </span>
            </div>
          </div>
          <RevenueChannelFilter
        ...
        {opexForPeriod > 0 && (
          <div className='rounded-xl border border-amber-200 bg-amber-50/60 px-4 py-3 flex flex-wrap items-center justify-between gap-3 text-sm animate-in slide-in-from-top-2'>
            <p className='text-amber-900'>
              <strong>{formatPrice(opexForPeriod)}</strong> operating costs
              logged for {year} are deducted in the profit flow when viewing
              this year.
            </p>
            <Link
              href='/admin/expenses'
            ...
            <div className='space-y-6 sm:space-y-8'>
              ...
              {periodData?.channelMix && channel === "all" && (
                <div className='rounded-[1.5rem] border border-gray-200/80 bg-white p-5 sm:p-6 shadow-sm'>
                  ...
                  <p className='text-xs text-gray-500 mb-4'>
                    Website · stall/offline POS · B2B wholesale ·{" "}
                    {periodData.label}
                  </p>
                  <ChannelSplitBar
              ...
              {channel !== "all" && periodData && (
                <div className='rounded-xl border border-brand-200 bg-brand-50/70 px-4 py-3 text-sm text-brand-900'>
                  ...
                  <strong>All channels</strong> to compare online, offline, and
                  B2B together.
                </div>
              )}
              ...
              <div className='grid grid-cols-1 xl:grid-cols-2 gap-6'>
                ...
                <div className='rounded-[1.5rem] border border-gray-100 bg-white p-5 sm:p-6 shadow-sm hover:shadow-md transition-shadow duration-300'>
                  ...
                  <p className='text-xs text-gray-500 mb-4'>
                    {periodData?.label ?? "Selected period"} - gross minus
                    refunds
                  </p>
                  ...
                  <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2'>
                    <div>
                      ...
                      <p className='text-xs text-gray-500 mt-1'>
                        {periodData?.label ?? "Period"} - tap a row for SKU
                        breakdown
                      </p>
                    </div>
              ...
              <div className='grid grid-cols-1 xl:grid-cols-12 gap-6 xl:gap-8'>
                <section className='xl:col-span-5 space-y-4'>
                  ...
                  <div className='grid grid-cols-2 gap-3'>
                    <MiniStat
                      label='Gross revenue'
                      value={formatPrice(fin.grossRevenue)}
                      sub='Checkout totals'
                    />
                    <MiniStat
                      label='Net revenue'
                      value={formatPrice(fin.netRevenue)}
                      accent='navy'
                      sub='After refunds'
                    />
                    <MiniStat
                    ...
                    <MiniStat
                      label='Gross profit'
                      value={formatPrice(fin.grossProfit)}
                      accent='emerald'
                      sub='− COGS'
                    />
                    <MiniStat
                ...
                <section className='xl:col-span-7 space-y-4'>
                  ...
                  <p className='text-[11px] text-gray-500 -mt-2'>
                    Catalog products only · grouped by category / subcategory
                    (Premium prefixed) · offline manual lines excluded
                  </p>
                  <div className='rounded-[1.5rem] border border-gray-100 bg-white p-5 sm:p-6 shadow-sm hover:shadow-md transition-shadow duration-300 min-h-[200px]'>
                    {categoryProfit.length === 0 ?
                      <p className='text-sm text-gray-500 py-8 text-center'>
                        No category profit data for this period.
                      </p>
            ...
            <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
              <section className='rounded-[1.5rem] border border-gray-100 bg-white p-5 sm:p-6 shadow-sm hover:shadow-md transition-shadow duration-300 space-y-5'>
                ...
                <div className='grid grid-cols-2 gap-3'>
                  ...
                  <LeakCard
                    ...
                    value={formatPrice(shippingTotal)}
                  />
                ...
                <div>
                  ...
                  <PaymentMethodMixPanel
                    rows={
                      periodData?.paymentMethodMix ??
                      analytics.paymentMethodMix ??
                      []
                    }
                    grossTotal={gross}
                  />
              ...
              <section className='space-y-4'>
                ...
                <p className='text-[11px] text-gray-500 leading-relaxed px-1 bg-white/50 backdrop-blur-sm p-3 rounded-xl border border-gray-200/50'>
                  Inventory panel shows <strong>potential</strong> margin on
                  unsold stock. Product ledger shows <strong>realized</strong>{" "}
                  margin for the selected period.
                </p>
              </section>
            </div>
          )}
