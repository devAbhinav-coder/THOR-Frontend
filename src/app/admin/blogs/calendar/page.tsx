...
import {
  useAdminAiStatus,
  aiErrorMessage,
} from "@/components/admin/ai/useAdminAi";
      ...
      blogContentPlanApi.bulkCreate(
        items.map((i) => ({
          ...
          notes: [
            i.notes,
            i.trendReason,
            i.festivalHook ? `Festival: ${i.festivalHook}` : "",
          ]
            .filter(Boolean)
            .join(" | "),
        })),
    ...
    <div className='p-6 max-w-5xl mx-auto space-y-8'>
      ...
      {status?.blogEnabled && (
        <div className='rounded-2xl border border-violet-200 bg-gradient-to-br from-violet-50/90 to-white p-5 space-y-4 shadow-sm'>
          <div className='flex items-start gap-2'>
            ...
            <div>
              <h2 className='text-sm font-bold text-violet-900'>
                AI Content Planner (Gemini + Trends)
              </h2>
              <p className='text-xs text-violet-800/80 mt-1 leading-relaxed'>
                India seasonal trends, upcoming festivals (Diwali, Rakhi,
                Navratri…), shop categories &amp; published blog gaps se auto
                plan. Google Trends API abhi alpha hai - hum local trend seeds
                use karte hain.
              </p>
            </div>
          </div>
          ...
          {aiSummary && (
            <p className='text-sm text-gray-700 bg-white/80 rounded-xl p-3 border border-violet-100'>
              {aiSummary}
            </p>
          )}
        ...
        <table className='w-full text-sm text-left'>
          ...
          <tbody className='divide-y divide-gray-100'>
            ...
            : plans.length === 0 ?
              <tr>
                <td colSpan={5} className='px-4 py-8 text-center text-gray-400'>
                  No planned topics - AI plan generate karo upar se
                </td>
              </tr>
            : plans.map((plan) => (
                <tr key={plan._id} className='hover:bg-gray-50/80'>
                  <td className='px-4 py-3 whitespace-nowrap'>
                    {new Date(plan.plannedDate).toLocaleDateString("en-IN", {
                      day: "numeric",
                      month: "short",
                      year: "numeric",
                    })}
