...
export default function AdminDashboardPage() {
  ...
  const [invValuation, setInvValuation] =
    useState<InventoryValuationOverall | null>(null);
  ...
  const loadAnalytics = useCallback(async (silent = false) => {
    ...
    try {
      ...
      if (valRes.status === "fulfilled") {
        const overall = (
          valRes.value.data as { overall?: InventoryValuationOverall }
        ).overall;
        ...
        <div className='p-4 sm:p-6 xl:p-8 space-y-8 max-w-[1600px] mx-auto'>
          ...
          <div className='grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 gap-3'>
            {[...Array(4)].map((_, i) => (
              <div
                key={i}
                className='h-[6.5rem] rounded-2xl bg-gray-100 animate-pulse'
              />
            ))}
          </div>
  ...
  const returnReasonTotal =
    (analytics.refundsByReason || []).reduce((s, r) => s + (r.count || 0), 0) ||
    1;
          ...
          <div className='relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-6'>
            <div>
              ...
              <h1 className='text-3xl sm:text-4xl lg:text-5xl font-bold font-serif text-white tracking-tight leading-tight'>
                ...
                <span className='text-brand-400 inline-block animate-bounce-slow'>
                  ✦
                </span>
              </h1>
              <p className='mt-3 text-navy-200 max-w-xl text-sm sm:text-base leading-relaxed font-medium'>
                Here's your operations pulse for today. Keep an eye on your
                fulfillment queue and catalogue health to maintain momentum.
              </p>
            </div>
            ...
            <div className='space-y-6 sm:space-y-8'>
              ...
              <div className='grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4'>
                ...
                <AdminMetricCard
                  ...
                  sub={`${overview.newUsersThisMonth} new MTD · website signups`}
                  icon={Users}
                  className='hover:-translate-y-1 transition-transform duration-300'
                />
                <AdminMetricCard
              ...
              <div className='bg-white rounded-[1.5rem] border border-gray-100 shadow-sm hover:shadow-md transition-shadow duration-300 overflow-hidden flex flex-col'>
                <div className='flex flex-col sm:flex-row sm:items-center justify-between px-6 py-5 border-b border-gray-100 gap-4'>
                  ...
                  <Link
                    href='/admin/orders'
                    className='inline-flex items-center justify-center rounded-xl bg-gray-50 border border-gray-200 px-4 py-2 text-xs font-bold text-gray-700 hover:bg-gray-100 hover:text-gray-900 transition-colors shrink-0'
                  >
                ...
                <div className='sm:hidden divide-y divide-gray-100'>
                  {analytics.recentOrders.map((order) => {
                    ...
                    return (
                      <div
                        key={order._id}
                        className='px-5 py-4 hover:bg-gray-50/50 transition-colors group relative'
                      >
                        <div className='flex items-start justify-between gap-3'>
                          ...
                          <span
                            className={`text-[10px] font-bold px-2 py-1 rounded-md uppercase tracking-wider ${getOrderStatusColor(order.status)}`}
                          >
                            {order.status}
                          </span>
                        </div>
                        <div className='mt-4 flex items-center justify-between pl-[3.25rem]'>
                          ...
                          <p className='text-sm font-bold font-serif text-gray-900 bg-gray-50 px-2 py-1 rounded-md border border-gray-100'>
                    ...
                    <tbody className='divide-y divide-gray-50'>
                      {analytics.recentOrders.map((order) => {
                        ...
                        return (
                          <tr
                            key={order._id}
                            className='group hover:bg-brand-50/30 transition-all duration-200'
                          >
                            ...
                            <td className='px-6 py-4 text-right'>
                              <span
                                className={`inline-flex items-center text-[10px] font-bold px-2.5 py-1 rounded-md uppercase tracking-wider shadow-sm border border-black/5 ${getOrderStatusColor(order.status)}`}
                              >
                                {order.status}
                              </span>
                            </td>
              ...
              <div className='grid grid-cols-2 lg:grid-cols-5 gap-4'>
                <div className='bg-white rounded-2xl p-5 border border-gray-100 shadow-sm hover:-translate-y-1 transition-transform duration-300'>
                  ...
                  <p className='text-2xl font-bold font-serif text-gray-900 mt-2'>
                    {formatPrice(overview.avgOrderValue ?? 0)}
                  </p>
                ...
                <div className='bg-gradient-to-br from-brand-50 to-white rounded-2xl p-5 border border-brand-100 shadow-sm ring-1 ring-brand-100/50 hover:-translate-y-1 transition-transform duration-300'>
                  ...
                  <p className='text-2xl font-bold font-serif text-brand-950 mt-2'>
                ...
                <div className='bg-gradient-to-br from-brand-50 to-white rounded-2xl p-5 border border-brand-100 shadow-sm ring-1 ring-brand-100/50 hover:-translate-y-1 transition-transform duration-300'>
                  ...
                  <p className='text-2xl font-bold font-serif text-brand-950 mt-2'>
                    {formatPrice(overview.revenueToday ?? 0)}
                  </p>
                  <p className='text-xs text-brand-600/70 mt-1'>
                    Gross same window
                  </p>
                ...
                <div className='bg-white rounded-2xl p-5 border border-gray-100 shadow-sm hover:-translate-y-1 transition-transform duration-300'>
                  <p className='text-[11px] text-amber-600 uppercase tracking-widest font-semibold'>
                    Fulfilment queue
                  </p>
                  <p className='text-2xl font-bold font-serif text-amber-900 mt-2'>
                    {overview.pendingFulfillmentCount ?? 0}
                  </p>
                ...
                <div className='bg-white rounded-2xl p-5 border border-gray-100 shadow-sm hover:-translate-y-1 transition-transform duration-300'>
                  ...
                  <p className='text-xs text-gray-400 mt-1'>
                    {overview.reviewsThisMonth ?? 0} this month
                  </p>
                </div>
              </div>
                      ...
                      <div className='flex items-center gap-3'>
                        ...
                        <div>
                          ...
                          <p className='text-xs text-gray-500 mt-1 max-w-sm leading-relaxed'>
                            Cash returned to customers. Gross revenue uses
                            original order totals; net retained uses gross −
                            refunds.
                          </p>
                        </div>
                      </div>
                  ...
                  <div className='flex items-center justify-between p-5 bg-gray-50/80 rounded-2xl gap-4 flex-wrap border border-gray-100'>
                    <div>
                      ...
                      <p className='text-2xl font-serif font-bold text-gray-900 mt-2'>
                        {formatPrice(overview.refundedAmount ?? 0)}
                      </p>
                    </div>
                    <div className='text-right'>
                      ...
                      <p className='text-2xl font-serif font-bold text-gray-900 mt-2'>
                        {overview.refundedOrdersCount ?? 0}
                      </p>
                    </div>
                ...
                <div className='bg-white rounded-[1.5rem] p-6 border border-gray-100 shadow-sm hover:shadow-md transition-shadow duration-300'>
                  ...
                  <div className='space-y-4'>
                    ...
                    : analytics.refundsByReason.map((reason, i) => (
                        ...
                        >
                          <span className='text-sm font-semibold text-gray-700 truncate w-1/2'>
                            {reason._id}
                          </span>
                          <div className='flex flex-1 justify-end items-center gap-4'>
                            <div className='w-32 h-2 bg-gray-100 rounded-full overflow-hidden hidden sm:block'>
                              <div
                                ...
                                style={{
                                  width: `${(reason.count / returnReasonTotal) * 100}%`,
                                }}
                              />
                            </div>
                            <span className='text-sm font-bold text-gray-900 w-6 text-right tabular-nums'>
                              {reason.count}
                            </span>
                          </div>
                        </div>
              ...
              {topViewed.length > 0 && (
                <div className='hover:-translate-y-1 transition-transform duration-300'>
                  <MarketingPulseStrip
                    topViewed={topViewed}
                    attributedOrders={
                      analytics.marketingInsights?.attributedOrders
                    }
                  />
                </div>
              )}
                ...
                <div className='bg-white rounded-[1.5rem] p-6 border border-gray-200/80 shadow-sm hover:shadow-md transition-shadow duration-300'>
                  <div className='flex items-center justify-between mb-5'>
                    <div>
                      ...
                      <p className='text-[11px] text-gray-500 mt-0.5'>
                        Paid catalog · no offline manual
                      </p>
                    </div>
                    <Link
                      href='/admin/analytics'
                      className='text-xs font-bold text-brand-600 hover:text-brand-700 hover:underline bg-brand-50 px-3 py-1.5 rounded-full'
                    >
                      Full ranking &rarr;
                    </Link>
                  </div>
                  <div className='space-y-4'>
                    {analytics.topProducts.map((product, i) => (
                      ...
                      >
                        <span className='text-xs font-bold text-gray-400 w-4 tabular-nums text-center'>
                          {i + 1}
                        </span>
                        ...
                        <div className='flex-1 min-w-0'>
                          ...
                          <p className='text-xs font-medium text-gray-500 mt-0.5'>
                            {product.totalSold} units · paid orders
                          </p>
                        </div>
