...
import { formatDate, formatPrice, cn, getOrderStatusColor, getPaymentStatusColor } from '@/lib/utils';
...
function tierBadge(segment: UserInsight['metrics']['userSegment']) {
  ...
  const t = map[segment] ?? { label: String(segment).replace(/_/g, ' '), className: 'bg-gray-100 text-gray-700 border-gray-200' };
  return (
    <span
      className={cn(
...
function UserAvatar({ user, size = 'md' }: { user: User; size?: 'sm' | 'md' | 'lg' }) {
  const dim = size === 'lg' ? 'h-16 w-16 text-xl' : size === 'sm' ? 'h-9 w-9 text-sm' : 'h-11 w-11 text-base';
  return (
    <div className="relative shrink-0">
      ...
      >
        {user.avatar ? (
          <Image src={user.avatar} alt="" width={64} height={64} className="h-full w-full object-cover" />
        ) : (
...
export default function AdminUsersPage() {
  ...
  }, [userPage, adminPage, loadUsers, loadAdmins]);
  ...
  useEffect(() => {
    adminApi.getUserDirectoryStats().then((r) => setDirectoryStats(r.data)).catch(() => {});
  }, []);
            ...
            >
              <div className="flex items-start justify-between gap-2">
                <div>
                  ...
                  <p className="mt-1.5 text-2xl font-bold text-gray-900 tabular-nums tracking-tight">{card.value}</p>
          ...
          <div className="px-4 sm:px-6 py-5 border-b border-gray-100 bg-gradient-to-r from-navy-950/5 via-white to-white">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="font-serif text-lg font-bold text-gray-900 tracking-tight">Team & admins</h2>
          ...
          <div className="sm:hidden divide-y divide-gray-100">
            {isLoadingAdmins ? (
              ...
              <p className="px-4 py-10 text-center text-sm text-gray-500">
                {admins.length === 0 ? 'No admin accounts yet.' : 'No matches on this page.'}
              </p>
            ) : (
              filteredAdmins.map((user) => (
                <div key={user._id} className="px-4 py-4 hover:bg-gray-50/80 transition-colors">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-[11px] text-gray-400 mt-0.5 tabular-nums">{user.phone || '—'}</p>
                      </div>
                    </div>
                    <Badge variant={user.isActive ? 'success' : 'error'} className="text-[10px] shrink-0">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    ...
                    <button
                      ...
                      disabled={statusTogglingId === user._id || user._id === currentUserId}
                      title={user._id === currentUserId ? 'Another admin must change your status' : undefined}
                      className={cn(
                ...
                {isLoadingAdmins ? (
                  ...
                  filteredAdmins.map((user) => (
                    <tr key={user._id} className="hover:bg-brand-50/20 transition-colors group">
                      ...
                      <td className="px-5 py-3.5 text-gray-600 tabular-nums">{user.phone || '—'}</td>
        ...
        <section className="rounded-2xl border border-gray-200/80 bg-white shadow-[0_20px_50px_-28px_rgba(15,23,42,0.18)] overflow-hidden">
          <div className="px-4 sm:px-6 py-5 border-b border-gray-100 bg-gradient-to-r from-brand-50/40 via-white to-white">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="font-serif text-lg font-bold text-gray-900 tracking-tight">Customers</h2>
                <p className="text-xs text-gray-500 mt-0.5">
                  {pagination.total} in directory · search filters this page only
                </p>
              </div>
            </div>
            <div className="mt-4 max-w-md">
          ...
          <div className="sm:hidden divide-y divide-gray-100">
            {isLoading ? (
              ...
              <p className="px-4 py-10 text-center text-sm text-gray-500">
                {users.length === 0 ? 'No customers yet.' : 'No matches on this page.'}
              </p>
            ) : (
              filtered.map((user) => (
                <div key={user._id} className="px-4 py-4 hover:bg-gray-50/80 transition-colors">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <p className="text-sm font-semibold text-gray-900 truncate">{user.name}</p>
                        <p className="text-xs text-gray-500 truncate">{user.email}</p>
                        <p className="text-[11px] text-gray-400 mt-0.5">{formatDate(user.createdAt)}</p>
                      </div>
                    </div>
                    <Badge variant={user.isActive ? 'success' : 'error'} className="text-[10px] shrink-0">
                      {user.isActive ? 'Active' : 'Blocked'}
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-50">
                {isLoading ? (
                  ...
                  filtered.map((user) => (
                    <tr key={user._id} className="hover:bg-brand-50/15 transition-colors">
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-3">
                          <UserAvatar user={user} size="sm" />
                          <div className="min-w-0">
                      ...
                      <td className="px-5 py-3.5">
                        <div className="flex justify-end gap-1.5 flex-wrap">
                          ...
                          >
                            {statusTogglingId === user._id ? '…' : user.isActive ? 'Block' : 'Unblock'}
                          </button>
                          <button
                            type="button"
                            onClick={() => promoteToAdmin(user._id)}
          ...
          <div className="relative w-full sm:max-w-lg md:max-w-2xl bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl max-h-[92vh] overflow-hidden flex flex-col border border-gray-100">
            <div className="px-5 sm:px-7 pt-6 pb-5 bg-gradient-to-br from-navy-950 via-navy-900 to-brand-900 text-white shrink-0">
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-4 min-w-0">
                  ...
                  <div className="min-w-0">
                    <p className="text-[11px] font-semibold uppercase tracking-wider text-white/60">Customer profile</p>
                    <h3 className="text-xl font-bold font-serif truncate">{selectedUser.name}</h3>
            ...
            <div className="p-5 sm:p-7 overflow-y-auto flex-1 space-y-6 bg-gradient-to-b from-gray-50/50 to-white">
              {loadingInsight ? (
                ...
                <>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    {[
                      ...
                      { label: 'Spent', v: formatPrice(insight.metrics.totalSpent) },
                      { label: 'AOV', v: formatPrice(insight.metrics.avgOrderValue) },
                    ].map((row) => (
                      <div
                        key={row.label}
                      ...
                      >
                        ...
                        <p className="mt-1 text-lg font-bold text-gray-900 tabular-nums">{row.v}</p>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {insight.metrics.lastOrderAt && (
                      <span className="text-xs text-gray-500 inline-flex items-center gap-1.5 rounded-full bg-gray-100 px-2.5 py-1">
                        <Clock3 className="h-3.5 w-3.5" /> Last order {formatDate(insight.metrics.lastOrderAt)}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-[10px] text-gray-400">(team only)</span>
                    </div>
                    <textarea
                      value={noteDraft}
                  ...
                  <div className="rounded-2xl border border-gray-200 bg-white overflow-hidden shadow-sm">
                    ...
                    <div className="divide-y divide-gray-100 max-h-64 overflow-y-auto">
                      {insight.orders.length === 0 ? (
                        <p className="px-4 py-8 text-sm text-gray-500 text-center">No orders yet.</p>
                      ) : (
                        insight.orders.map((o) => (
                          <div key={o._id} className="px-4 py-3 flex items-center justify-between gap-3 hover:bg-gray-50/80">
                            <div className="min-w-0">
                            ...
                            <div className="text-right shrink-0">
                              <p className="text-sm font-bold text-gray-900 tabular-nums">{formatPrice(o.total)}</p>
                              <Link
                ...
                <p className="text-sm text-gray-500 text-center py-8">Could not load activity.</p>
              )}
            </div>
          </div>
        </div>
