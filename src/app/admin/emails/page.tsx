...
const audienceMeta: Record<
  Audience,
  { icon: ReactElement; label: string; hint: string }
> = {
  users: {
...
export default function AdminEmailsPage() {
  ...
  const sendCampaign = async () => {
    ...
    if (
      ...
      stats &&
      !stats.delivery.resendConfigured
    ) {
      toast.error(
        "Email is not configured on the server (RESEND_API_KEY). Use notifications only or configure Resend.",
      );
      return;
    }
        ...
        channels.includes("email") && (
          <div className='rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 flex gap-3 text-sm text-amber-900'>
            ...
            <p>
              ...
              on the server. In-app and browser push still work for logged-in
              users who allowed notifications.
            </p>
          </div>
        )}
            ...
            <div className='grid grid-cols-1 sm:grid-cols-3 gap-2'>
              {(
                ...
                return (
                  ...
                  >
                    <Icon
                      className={`h-5 w-5 ${on ? "text-brand-600" : "text-gray-400"}`}
                    />
          ...
          {channels.includes("email") && audience !== "admins" && (
            <label className='flex items-start gap-3 rounded-xl border border-gray-100 bg-gray-50/80 px-3 py-2.5 cursor-pointer'>
              ...
              <span className='text-sm text-gray-700'>
                Also email <strong>offline POS leads</strong> (emails from
                walk-in orders without a full account yet).
              </span>
            </label>
          )}
              ...
              <div className='space-y-3 text-sm'>
                <div className='flex justify-between gap-2'>
                  ...
                  <span className='font-semibold tabular-nums'>
                    {stats.accountUsers.toLocaleString()}
                  </span>
                </div>
                {channels.includes("email") &&
                  includeOfflineLeads &&
                ...
                <div className='rounded-xl bg-gray-50 border border-gray-100 p-3 text-xs text-gray-600 leading-relaxed space-y-1'>
                  <p className='inline-flex items-center gap-1 text-emerald-700 font-medium'>
                    ...
                    Background queue{" "}
                    {stats.delivery.redisEnabled ? "(Redis)" : "(inline)"}
                  </p>
                  <p>
                    Emails send ~10 per batch with 1–2s spacing (safe for Resend
                    limits). 100 emails ≈ few minutes; 10,000+ runs for hours
                    without blocking admin.
                  </p>
                  <p>
                    Browser push only reaches users who allowed notifications on
                    the site.
                  </p>
                </div>
              </div>
            : <p className='text-sm text-gray-400'>
                Select a channel to see reach.
              </p>
            }
          </div>
