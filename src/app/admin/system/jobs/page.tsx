...
export default function AdminJobHealthPage() {
  ...
  const [infrastructure, setInfrastructure] =
    useState<InfrastructureReport | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setInfrastructure(
        (res.data.infrastructure as InfrastructureReport) ?? null,
      );
    ...
    <div className='p-4 sm:p-6 max-w-[1400px] mx-auto min-h-[calc(100dvh-4rem)] bg-[#FAF9F6] space-y-4'>
            ...
            {(
              ...
              >
                ...
                <p
                  className={cn(
                    "mt-0.5 capitalize",
                    STATUS_COLOR[check.status] ?? "text-gray-600",
                  )}
                >
                  {check.status}
                </p>
      ...
      : rows.length === 0 ?
        <div className='rounded-xl border bg-white p-8 text-center text-sm text-gray-500'>
          ...
          No job runs recorded yet. Jobs report health after their first
          execution in worker mode.
        </div>
              ...
              <tbody className='divide-y divide-gray-100'>
                {rows.map((row) => (
                  <tr key={row.name} className='hover:bg-gray-50/80'>
                    ...
                    <td className='px-4 py-3 tabular-nums text-xs'>
                      {row.runCount} /{" "}
                      <span className='text-red-600'>{row.errorCount}</span>
                      {row.lastCount != null ?
                        <span className='text-gray-400 ml-1'>
                          · {row.lastCount} processed
                        </span>
                      : null}
                    </td>
                    <td className='px-4 py-3'>
                      {row.lastError ?
                        <span
                          className='inline-flex items-center gap-1 text-xs text-red-700 max-w-xs truncate'
                          title={row.lastError}
                        >
