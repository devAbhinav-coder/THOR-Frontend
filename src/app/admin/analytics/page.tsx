...
import {
  formatRevenueGrowthSub,
  revenueGrowthBadgeValue,
...
export default function AnalyticsPage() {
  ...
  const avgConversion = useMemo(() => {
    ...
    if (o?.storefrontConversionPercent != null) {
      return o.storefrontConversionPercent;
    }
    const views = o?.totalPdpViews ?? 0;
    const sold = o?.catalogUnitsSold ?? 0;
    if (views > 0) return Math.round((sold / views) * 10000) / 100;
    return 0;
  }, [analytics?.overview]);
          ...
          items={[
            {
              ...
              href: "/admin/revenue",
              description:
                "Gross, refunds, net retained, payment mix, and tax.",
            ...
            },
            {
              ...
              href: "/admin",
              description:
                "Today’s orders, fulfilment queue, and recent activity.",
              ...
              <div className='rounded-xl border border-gray-200 bg-white p-3 sm:p-4 shadow-sm'>
                ...
                <div className='grid grid-cols-2 sm:grid-cols-4 gap-2'>
                  <LeakCard
                    label='Admin sales'
                    value={formatPrice(
                      overview.saleDiscountTotal ??
                        offerMtd?.sales.discountTotal ??
                        0,
                    )}
                  ...
                  <LeakCard
                    label='Auto offers'
                    value={formatPrice(
                      overview.promotionDiscountMTD ??
                        offerMtd?.promotions.discountTotal ??
                        0,
                    )}
                  ...
                  <LeakCard
                    label='Coupons'
                    value={formatPrice(
                      overview.couponDiscountMTD ??
                        offerMtd?.coupons.discountTotal ??
                        0,
                    )}
              ...
              {analytics.snapshotOverview && (
                <div className='rounded-xl border border-emerald-200/60 bg-emerald-50/40 p-3 shadow-sm'>
                  <h2 className='text-xs font-semibold text-gray-900 mb-2'>
                    30-day snapshot totals (
                    {analytics.snapshotOverview.completedDaysFromSnapshots} days
                  ...
                  <div className='grid grid-cols-2 sm:grid-cols-3 gap-2 text-sm'>
                    <div>
                      ...
                      <p className='font-semibold tabular-nums'>
                        {formatPrice(analytics.snapshotOverview.totals.revenue)}
                      </p>
                    </div>
                    <div>
                      <p className='text-[10px] uppercase text-gray-500'>
                      ...
                      <p className='font-semibold tabular-nums'>
                        {analytics.snapshotOverview.totals.orders}
                      </p>
                    </div>
                    <div>
                      <p className='text-[10px] uppercase text-gray-500'>
                      ...
                      <p className='font-semibold tabular-nums'>
                        {analytics.snapshotOverview.totals.siteVisits}
                      </p>
                    </div>
                  </div>
                </div>
          ...
          {activeTab === "catalogue" && (
            <div className='space-y-3'>
              <StorefrontDemandSection
                ...
                catalogUnitsSold={overview.catalogUnitsSold}
                storefrontConversionPercent={overview.storefrontConversionPercent}
              />
              ...
              <div className='grid grid-cols-1 xl:grid-cols-12 gap-3 items-start'>
                <div className='xl:col-span-8 rounded-xl border border-gray-200 bg-white p-3 shadow-sm'>
                  <CategoryRevenueBarChart
                    ...
                    subtitle='Paid catalog lines · category / subcategory · Premium prefix'
                  />
                </div>
                <div className='xl:col-span-4 rounded-xl border border-gray-200 bg-white p-3 shadow-sm'>
                  ...
                  <p className='text-[11px] text-gray-500 mb-2'>
                    Catalog lines only · offline manual orders excluded
                  </p>
            ...
            <div className='space-y-3'>
              <div className='grid grid-cols-2 lg:grid-cols-4 gap-2'>
                <AdminMetricCard
                  ...
                  sub='Registered online buyers only'
                  icon={Users}
                  variant='navy'
                />
                <AdminMetricCard
                ...
                <AdminMetricCard
                  ...
                  sub={`${overview.totalUsers || 0} website accounts · POS leads excluded`}
                  icon={UserPlus}
                />
                <AdminMetricCard
                  compact
                  ...
                  sub='Online paid · registered shoppers'
                  icon={ShoppingBag}
                />
              </div>
                ...
                <div className='lg:col-span-8 rounded-xl border border-navy-800 bg-gradient-to-br from-navy-900 to-navy-950 p-3 shadow-lg text-white'>
                  ...
                  <div className='grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2'>
                    {[
                      ...
                      {
                        label: "Sale savings",
                        value: formatPrice(overview.saleDiscountTotal || 0),
                        tone: "text-amber-300",
                      },
                      {
                        label: "Auto offers MTD",
                        value: formatPrice(overview.promotionDiscountMTD || 0),
                        tone: "text-brand-300",
                      },
                      {
                        label: "Coupons MTD",
                        value: formatPrice(overview.couponDiscountMTD || 0),
                        tone: "text-brand-400",
                      },
                      {
                        label: "Popup opens MTD",
                        value: String(offerMtd?.popup.impressions ?? 0),
                        tone: "text-white",
                      },
                      {
                      ...
                      <div key={item.label}>
                        <p className='text-[9px] text-white/50 uppercase mb-0.5'>
                          {item.label}
                        </p>
                        <p
                          className={`text-lg font-bold tabular-nums ${item.tone}`}
                        >
                          {item.value}
                        </p>
                      </div>
