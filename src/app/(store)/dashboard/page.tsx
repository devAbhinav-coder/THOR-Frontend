...
export default function DashboardPage() {
  ...
  const { data: recentOrders = [], isLoading } = useQuery<Order[]>({
    ...
    queryFn: () =>
      orderApi.getMyOrders({ limit: 5 }).then((r) => r.data.orders),
    staleTime: 1000 * 60,
  });

  const { data: summary } = useQuery({
    queryKey: ["dashboard-orders-summary"],
    queryFn: () => orderApi.getMyOrdersSummary().then((r) => r.data.summary),
    staleTime: 1000 * 60 * 5,
  });
  ...
  const statCards = [
    { label: "Total Orders", value: totalCount, href: "/dashboard/orders" },
    {
      ...
      href: "/dashboard/orders?filter=delivered",
    },
    {
      label: "In Progress",
        ...
        <section className='lg:col-span-8 flex flex-col gap-account-gutter'>
          ...
          {isLoading ?
            <div className='flex flex-col gap-4'>
              {[...Array(2)].map((_, i) => (
                <div
                  key={i}
                  className='h-40 bg-account-surface-container animate-pulse border border-account-outline-variant/20'
                />
              ))}
            </div>
          : recentOrders.length === 0 ?
            <div className='bg-account-surface-container-lowest border border-account-outline-variant/30 p-10 text-center'>
              ...
              <p className='text-account-on-surface-variant font-medium'>
                No orders yet
              </p>
              <Link
                href='/shop'
                className='mt-4 inline-flex items-center gap-1.5 text-sm font-semibold text-account-secondary hover:underline'
              >
